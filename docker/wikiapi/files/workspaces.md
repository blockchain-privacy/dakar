# Workspaces

A Workspace is a powerful graph visualization tool designed to enhance your analysis of blockchain transactions and address clusters. It provides users with an intuitive interface to explore complex relationships and patterns within the blockchain, enabling deeper insights into transaction behaviors and structures.

![example graph](img/graph.webp)
*A workspace showing the connections between transactions and address clusters.*

## Key Features

### Visualize Connections Between Transactions and Address Clusters
A Workspace allows users to visualize the intricate connections between various transactions and address clusters. This feature helps in understanding how different entities interact within the blockchain ecosystem, making it easier to identify relationships and potential patterns of behavior.

### CoinJoin Heuristics
CoinJoin heuristics are analytical tools used to identify potential senders and receivers involved in CoinJoin transactions, employing distinct methodologies to narrow down the pool of possible candidates and enhancing the accuracy of transaction analysis. By applying these heuristics, users can gain insights into the relationships and interactions within CoinJoin transactions, ultimately improving their understanding of privacy-enhancing techniques in blockchain networks.

### Transaction Graph Selector
Transaction graph selectors allow users to select transactions by starting from a specific transaction and traversing several steps forward or backward in the transaction graph. This feature makes it easy to explore the relationships between transactions, helping users understand how funds flow and how different transactions are connected.

### Transaction Property Selector

The Transaction Property Selector is a powerful search feature that enables users to find transactions based on specific criteria. Users can search for transactions using the following properties:

- **Time**: Filter transactions based on the date and time they occurred.
- **Transaction Type**: Search for transactions by their type, such as standard transactions, CoinJoin transactions, or other categories.
- **Amount**: Find transactions that have a specific input or output amount range.
- **Amount Sum**: Find transactions that have a specific input or output amount sum range.
