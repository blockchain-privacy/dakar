...
func (c APIModule) check() error {
	if c.KratosPublicEndpoint == "" || c.KratosAdminEndpoint == "" {
		return serror.NewStackErrorStr("http module config invalid, not all fields are filled")
	}
...
func isCrawling(db external.Database) (bool, error) {
	...
	if err != nil {
		...
		return true, serror.NewStackErrorStr("was not able to get crawling status successfully")
	}
...
func waitForRPCClient(client external.RPCClient) error {
	...
	for i := range maxRetries {
		...
		if strings.Contains(err.Error(), "status code: 401") {
			return serror.NewStackErrorf("Authentication error: %w", err)
		}
	...
	return serror.NewStackErrorStr("RPC client is not ready to receive requests")
}
...
func shutdownServer(srv *http.Server) {
	...
	if err := srv.Shutdown(ctx); err != nil {
		warn(serror.NewStackErrorf("Server was shutdown and returned error: %w", err))
	}
}
...
func newKratosClient(endpoint string) (*ory.APIClient, error) {
	if endpoint == "" {
		return nil, serror.NewStackErrorf("endpoint is invalid: %s", endpoint)
	}
...
func getKratosClient(publicEndpoint string, adminEndpoint string) (*ory.APIClient, *ory.APIClient, error) {
	...
	if !waitForKratos(auth) {
		return nil, nil, serror.NewStackErrorStr("kratos public endpoint is not ready to receive requests")
	}

	// check if public endpoint is alive
	if !waitForKratos(adminAuth) {
		return nil, nil, serror.NewStackErrorStr("kratos admin endpoint is not ready to receive requests")
	}
