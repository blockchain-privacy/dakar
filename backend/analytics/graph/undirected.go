...
func NewUndirectedGraph(numNodesHint int) *UndirectedGraph {
	return &UndirectedGraph{
		nodes: make(map[int64]graph.Node, numNodesHint),
		edges: make(map[int64]map[int64]graph.Edge, numNodesHint),
...
func (g *UndirectedGraph) UpdateNode(n graph.Node) {
	if _, exists := g.nodes[n.ID()]; !exists {
		g.nodeIDs.Use(n.ID())
	}
	g.nodes[n.ID()] = n
...
func (g *UndirectedGraph) Edges() graph.Edges {
	...
	for _, u := range g.edges {
		for _, e := range u {
			...
			id := e.From().ID()
			vid := e.To().ID()
			if _, ok := seen[[2]int64{id, vid}]; ok {
				continue
			}
			seen[[2]int64{id, vid}] = struct{}{}
			seen[[2]int64{vid, id}] = struct{}{}
...
// and are set to the nodes of the edge otherwise.
// It will panic if the IDs of the e.From and e.To are equal.
func (g *UndirectedGraph) SetEdgeWithoutOverwrite(e graph.Edge) {
	var (
		from = e.From()
		fid  = from.ID()
		to   = e.To()
