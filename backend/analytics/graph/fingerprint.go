...
// SpendingFingerprintChamfer returns a list of transaction uids which have a similar spending pattern
// and the number of mixing sessions of this transactions. Uses the Chamfer distance.
func SpendingFingerprintChamfer(g *ReversibleGraph, uid string) ([]FingerPrint, error) {
	// maximumDistance is the maximum distance between to earliest (lowest) input timestamp
	// of the root transaction and the timestamp of the compared transaction
	// 2 days = 60 * 60 * 24 * 2 = 172800 seconds
	const maximumDistance = 172800
	...
	for nodes.Next() {
		...
		fingerprints = append(fingerprints, FingerPrint{
			TransactionUID: txNode.String(),
			Score:          calcChamferDistance(g, rootTx, txNode),
		})
		sort.Slice(fingerprints, func(i, j int) bool {
			return fingerprints[i].Score > fingerprints[j].Score
		})
