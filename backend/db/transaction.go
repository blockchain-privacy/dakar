...
func (t *Transaction) CalculateTransactionFee() (err error) {
	...
	for _, e := range t.Inputs {
		if e.Amount == nil {
			return serror.NewStackErrorStr("amount is not set")
		}
...
func GetTransactionsOutputs(c external.Database, transactionHashes []string) (
	...
	for _, t := range transactionHashes {
		if !isValidQueryInput(t) {
			return nil, serror.NewStackErrorStr("invalid transaction hash")
		}
	}
	...
	if len(transactionHashes) != len(r.Transactions) {
		err = serror.NewStackErrorStr("number of returned transactions does not match number of requested transactions")
		return
	}
...
func GetTransactionByBlock(c external.Database, blockID uint64) (transactions []Transaction, err error) {
	...
	if len(r.Q) == 0 {
		err = serror.NewStackErrorf("block: %d: %w", blockID, ErrTransactionNotFound)
		return
	}
...
func GetTransaction(c external.Database, txHash string) (transaction Transaction, err error) {
	...
	if len(r.Q) == 0 {
		err = serror.NewStackError(ErrTransactionNotFound)
		return
	}
...
func GetFrontendTransactionAmounts(c external.Database, txUids []string) (txs []AmountTransaction, err error) {
	...
	for i, tx := range r.Transactions {
		if len(tx.Block) != 1 {
			err = serror.NewStackErrorf("multiple blocks returned for transaction %s", tx.Hash)
			return
		}
...
func UpdateTransactions(c external.Database, transactions []Transaction) error {
	...
	for _, tx := range transactions {
		if tx.UID == "" {
			return serror.NewStackErrorStr("uid is not set for transaction " + tx.String())
		}
	}
