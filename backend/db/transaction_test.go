...
func TestGetTransactionByBlock(t *testing.T) {
	...
	transactions, err := GetTransactionsByBlock(dbHandle, 1)
	require.Error(t, err)
	require.Nil(t, transactions)

	transactions, err = GetTransactionsByBlock(dbHandle, 60001)
	require.NoError(t, err)
...
func TestGetFrontendTransactionsByUID(t *testing.T) {
	...
	transactions, err := GetTransactionsByBlock(dbHandle, 60005)
	require.NoError(t, err)
